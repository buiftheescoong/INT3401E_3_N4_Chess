...
double quiescence_search(chess::Board& board, double alpha, double beta, bool is_maximising_player) {
    ...
    std::vector<chess::Move> captures = board.generate_legal_captures(chess::BB_ALL, chess::BB_ALL);
...
chess::Move minimax_root(int depth, chess::Board& board) {
    ...
    if (legal_moves.count() == 0) {
        std::cout << "minimax_root: No legal moves at depth " << depth << "! Returning null move." << std::endl;
        return chess::Move(0,0); // Không có nước đi nào
    }

    chess::Move best_move_found = chess::Move(0,0); // Rất quan trọng: Khởi tạo
    int best_value;
    bool is_white_turn = board.turn == chess::WHITE; // Hoặc board.turn là true/false

    if (is_white_turn) { // Lượt của Trắng (Maximizing)
        best_value = -999999; // Giá trị âm vô cùng nhỏ
        bool first_move = true;
        for (const chess::Move& move : legal_moves) {
            board.push(move);
            int value = minimax(depth - 1,board , -999999, 999999, false); // Gọi cho lượt của Đen
            board.pop();
            std::cout << "  Depth " << depth << " (W): Move " << move.uci() << ", eval: " << value << std::endl;

            if (first_move || value > best_value) { // Nếu là Maximizing player
                best_value = value;
                best_move_found = move;
                first_move = false;
            }
        }
    } else { // Lượt của Đen (Minimizing)
        best_value = 999999; // Giá trị dương vô cùng lớn
        bool first_move = true;
        for (const chess::Move& move : legal_moves) {
            board.push(move);
            int value = minimax(depth - 1,board, -999999, 999999, true); // Gọi cho lượt của Trắng
            board.pop();
            std::cout << "  Depth " << depth << " (B): Move " << move.uci() << ", eval: " << value << std::endl;

            if (first_move || value < best_value) { // Nếu là Minimizing player
                best_value = value;
                best_move_found = move;
                first_move = false;
            }
    ...
    std::cout << "minimax_root (depth " << depth << "): Best move found: " << best_move_found.uci() << " with value " << best_value << std::endl;
